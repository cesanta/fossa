...
static void send_error_result(struct ns_connection *nc, const char *msg) {
  ns_printf_http_chunk(nc, "Error: %s", msg);
  ns_send_http_chunk(nc, "", 0);  /* Send empty chunk, the end of response */
}

static void link_conns(struct ns_connection *nc1, struct ns_connection *nc2) {
  nc1->user_data = nc2;
  nc2->user_data = nc1;
}

static void unlink_conns(struct ns_connection *nc1) {
  struct ns_connection *nc2 = (struct ns_connection *) nc1->user_data;
...
/* S3 client handler */
static void s3_handler(struct ns_connection *nc, int ev, void *ev_data) {
  struct http_message *hm = (struct http_message *) ev_data;
  struct ns_connection *nc2 = (struct ns_connection *) nc->user_data;

  switch (ev) {
    case NS_HTTP_REPLY:
      if (nc2 != NULL) {
        ns_printf_http_chunk(nc2, "Error: %.*s", (int)hm->message.len,
                             hm->message.p);
        ns_send_http_chunk(nc2, "", 0);
      }
      unlink_conns(nc);
...
static void send_s3_request(struct ns_connection *nc, const char *file_name,
                            const char *file_data, const char *host,
                            const char *bucket) {
  char host_port[100];
  struct ns_connection *s3_conn;

  snprintf(host_port, sizeof(host_port), "%s:80", host);
  s3_conn = ns_connect(nc->mgr, host_port, s3_handler);
  ...
  if (s3_conn == NULL) {
    ...
    ns_set_protocol_http_websocket(s3_conn);
    ...
    ns_base64_encode((unsigned char *) sha1, sizeof(sha1), signature);
    ...
    ns_printf(s3_conn, "%s%s", req, file_data);
    /* S3 request sent, wait for a reply */
  }
}

static void handle_api_call(struct ns_connection *nc, struct http_message *hm) {
  char file_name[100], file_data[100], host[100], bucket[100];

  /* Get form variables */
  ns_get_http_var(&hm->body, "file_name", file_name, sizeof(file_name));
  ns_get_http_var(&hm->body, "file_data", file_data, sizeof(file_data));
  ns_get_http_var(&hm->body, "host", host, sizeof(host));
  ns_get_http_var(&hm->body, "bucket", bucket, sizeof(bucket));
