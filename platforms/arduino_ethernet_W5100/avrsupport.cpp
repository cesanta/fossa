...
/*
 * TODO(alashkin): should we use some code to find
 * free port?
 */
#ifndef PORT_BASE
...
/* at least ATmega2560 uses little-endian model */
...
static void DumpSocket(W5100_sock_t* s) {
  Serial.print("SrSN: ");
...
static W5100_sock_t* W5100_get_sock() {
  W5100_raw_sock_t s;
  unsigned int i;
...
static void W5100_complete_sock_init(W5100_sock_t* sock, uint16_t port) {
  if (sock->status != ssNotInited) {
    return;
  }

  if (port == 0) {
    sock->port = sock->sockid + PORT_BASE;
  } else {
    sock->port = port;
  }
...
static int W5100_is_send_complete(sock_t s) {
...
static int W5100_is_connect_complete(sock_t s) {
  W5100_sock_t* sock = (W5100_sock_t*) s;
  uint16_t snsr;
...
static void W5100_proceed_non_blocking(W5100_sock_t* sock) {
  switch (sock->status) {
    case ssSending:
      W5100_is_send_complete(sock);
      break;
    case ssConnecting:
      W5100_is_connect_complete(sock);
      break;
    default:
...
static int W5100_is_socket_readable(sock_t s) {
  W5100_sock_t* sock = (W5100_sock_t*) s;

  W5100_proceed_non_blocking(sock);
...
static int W5100_is_socket_writable(sock_t s) {
  W5100_sock_t* sock = (W5100_sock_t*) s;
...
static int W5100_is_socket_in_error(sock_t s) {
  W5100_sock_t* sock = (W5100_sock_t*) s;
...
int getsockopt(sock_t s, int level, int optname, char* optval, int* optlen) {
  ...
  int res = W5100_is_socket_in_error(s);
...
int sendto(sock_t s, const void* buf, size_t len, int flags,
  ...
  W5100_complete_sock_init(sock, 0);
...
int recv(sock_t s, char* buf, int len, int flags) {
  W5100_sock_t* sock = (W5100_sock_t*) s;
  int16_t avail_bytes = W5100.getRXReceivedSize(sock->sockid);
...
int send(sock_t s, const char* buf, int len, int flags) {
  W5100_sock_t* sock = (W5100_sock_t*) s;
  int16_t avail_bytes = W5100.getTXFreeSize(sock->sockid);
...
int recvfrom(sock_t s, char* buf, int len, int flags, struct sockaddr* from,
  ...
  uint16_t bytes_to_read, avail_bytes = W5100.getRXReceivedSize(sock->sockid);
  ...
  if (avail_bytes < sizeof(udp_head)) {
...
int bind(sock_t s, const struct sockaddr* name, int namelen) {
  ...
  W5100_complete_sock_init(sock, ntohs(addr->sin_port));
...
static void W5100_restart_listen(W5100_sock_t* sock) {
  W5100.execCmdSn(sock->sockid, Sock_CLOSE);
  W5100.writeSnIR(sock->sockid, 0xFF);
...
sock_t accept(sock_t s, struct sockaddr* name, int* addrlen) {
  ...
  /*
   * Once listening socket gets a connection
   * it drops listen
   * so, we need to create another socket
  ...
  for (;;) {
    ...
    if (readfds != NULL) {
      for (i = 0; i < readfds->fd_count; i++) {
        if (W5100_is_socket_readable(readfds->fd_array[i])) {
    ...
    if (writefds != NULL) {
      for (i = 0; i < writefds->fd_count; i++) {
        if (W5100_is_socket_writable(writefds->fd_array[i])) {
    ...
    if (exceptfds != NULL) {
      for (i = 0; i < exceptfds->fd_count; i++) {
        if (W5100_is_socket_in_error(exceptfds->fd_array[i])) {
