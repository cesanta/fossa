...
#define SPI_CLOCK_DIVIDER \
  6  // used to set the speed for the SPI bus; 6 == 14 Mhz on the Arduino Due
#else
#define SPI_CLOCK_DIVIDER \
  SPI_CLOCK_DIV2  // Don't set this to a slower speed (i.e. larger div value)
...
#define CHECK_PRINTER if (false)
    ...
    (defined(__arm__) && defined(__SAM3X8E__))
#define CC3K_DEFAULT_PRINTER &Serial
#else
#define CC3K_DEFAULT_PRINTER 0
#endif

#define SMART_CONFIG_TIMEOUT \
  60000  // how long to wait for smart config to complete
...
class Adafruit_CC3000_Client : public Client {
 public:
  ...
  // NOTE: If public functions below are added/modified/removed please make sure
  // to update the
...
// Ugly but necessary to include the server header after the client is fully
// defined.
// A forward reference in the server header won't cut it because the server
// needs to contain
// instances of the client.  The client definition above can be pulled into a
// separate
...
class Adafruit_CC3000 {
 public:
  Adafruit_CC3000(uint8_t csPin, uint8_t irqPin, uint8_t vbatPin,
                  uint8_t spispeed = SPI_CLOCK_DIVIDER,
                  Print *cc3kPrinter = CC3K_DEFAULT_PRINTER);
  bool begin(uint8_t patchReq = 0, bool useSmartConfigData = false,
             const char *_deviceName = NULL);
  ...
  bool setStaticIPAddress(uint32_t ip, uint32_t subnetMask,
                          uint32_t defaultGateway, uint32_t dnsServer);
  bool setDHCP();
  ...
  bool startSmartConfig(const char *_deviceName = NULL,
                        const char *smartConfigKey = NULL,
                        uint32_t = SMART_CONFIG_TIMEOUT);
  bool getIPConfig(tNetappIpconfigRetArgs *ipConfig);

  uint16_t ping(uint32_t ip, uint8_t attempts = 3, uint16_t timeout = 500,
                uint8_t size = 32);
